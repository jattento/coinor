...
extension GrokControlError: LocalizedError {
    var errorDescription: String? {
        ...
        case let .executablePathNotAbsolute(path):
            return "The Grok executable path must be absolute, but Coinor was given \(path)."
        ...
        case let .leaderSocketPathNotAbsolute(path):
            return "The Grok leader socket path must be absolute, but Coinor was given \(path)."
        ...
        case let .launchFailed(detail):
            return "Coinor could not start Grok: \(detail)"
        ...
        case let .executableVersionTimedOut(path, seconds):
            return "Coinor could not verify \(path): `grok --version` did not finish within \(Int(seconds)) seconds."
        case let .executableVersionFailed(path, status, diagnostics):
            let tail = diagnostics.isEmpty ? "" : " Grok reported: \(diagnostics)"
            return "Coinor could not verify \(path): `grok --version` exited with status \(status).\(tail)"
        case let .executableVersionEmpty(path):
            return "Coinor could not verify \(path): `grok --version` returned no version text."
        ...
        case let .unsupportedMethod(method):
            return "This Grok build does not support \(method), which Coinor requires."
        ...
        case let .malformedPayload(method, detail):
            return "Coinor could not read Grok's response to \(method): \(detail)"
        case let .incompatibleAgent(detail):
            return "This Grok build is not compatible with Coinor: \(detail)"
        ...
        case let .malformedFrame(detail):
            return "Coinor received a malformed message from Grok: \(detail)"
        case let .frameTooLarge(size):
            return "Coinor received an oversized \(size) byte message from Grok."
        }
    }
}
